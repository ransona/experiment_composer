...
def main():
    ...
    # reconstruction video + timeline timestamps (on-demand frames)
    recon_src = ReconstructionVideoSource(
        video_path=os.path.join(exp_dir_processed, "reconstruction", "session_recons.mp4"),
    ...
    recon_stim_src = ReconstructionVideoSource(
        video_path=os.path.join(exp_dir_processed, "reconstruction", "session_projections.mp4"),
        timestamps_path=os.path.join(exp_dir_processed, "reconstruction", "video_timeline.npy"),
        enable_temporal_filter=True,   # enable/disable
        temporal_window=3,             # frames on each side → 7-frame box
        enable_spatial_filter=True,    # enable/disable
